...
class DataPreprocessingToolkit(object):
    ...
    def filter_out_company_clients(df):
        ...
        :rtype: pd.DataFrame
        """
        filtered_df = df.loc[(df['is_company'] == 0)]
        return filtered_df
    ...
    def filter_out_long_stays(df):
        ...
        :rtype: pd.DataFrame
        """
        filtered_df = df.loc[(df['length_of_stay'] <= 21)]
        return filtered_df
    ...
    def filter_out_low_prices(df):
        ...
        :rtype: pd.DataFrame
        """
        filtered_df = df.loc[(df['accommodation_price'] > 50)]
        return filtered_df
    ...
    def add_weekend_stay(df):
        ...
        def contain_weekday(date_from, date_to):
            dates = pd.date_range(date_from,date_to-timedelta(days=1),freq='d').tolist()
            days = list(map(lambda x: x.dayofweek, dates))
            if 4 in days or 5 in days:
                return 'True'
            return 'False'
            
        df['weekend_stay'] = df[['date_from', 'date_to']].apply(lambda x: contain_weekday(x[0], x[1]), axis=1)
    ...
    def add_night_price(df):
        ...
        :rtype: pd.DataFrame
        """
        df['night_price'] = np.around(df['accommodation_price'] / df['length_of_stay'] / df['n_rooms'], 2)
        return df
    ...
    def aggregate_group_reservations(self, df):
        ...
        new_df = pd.merge(sum_cols, mean_cols, on='group_id')
        new_df = pd.merge(new_df, mode_cols, on='group_id')
        new_df = pd.merge(new_df, first_cols, on='group_id')
        
        new_df = pd.concat([non_group_reservations, new_df])
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        :rtype: pd.DataFrame
        """
        df['room_segment'] = df.groupby('room_group_id')['night_price'].transform('mean').apply(
